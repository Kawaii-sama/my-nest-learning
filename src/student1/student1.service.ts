...
export class Student1Service {
    ...
    async updateStudent1(id : string, data : Partial<Student1>): Promise<Student1 | null> {
        ...
        const updated = await this.student1Model.findByIdAndUpdate (id, {
            name : data.name ?? null,
            age : data.age ?? null,
    ...
    async deleteStudent1(id : string) :Promise <Student1 | null> {
        return this.student1Model.findByIdAndDelete(id).exec();
